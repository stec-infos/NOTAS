    ...
    void GenerarBackupMySQL() {
        ...
        if (resp == JFileChooser.APPROVE_OPTION) {//Si el usuario presiona aceptar; se genera el Backup
            try {
                ...
                //Process child = runtime.exec("C:\\Archivos de programa\\MySQL\\MySQL Server 5.6\\bin\\mysql -u "+Conexion.user+" -p"+Conexion.passw+" --default-character_set=utf8 "+Conexion.dbname);
                Process child = runtime.exec("C:\\xampp\\mysql\\bin\\mysql -u " + Conexion.user + " -p" + Conexion.passw + " --default-character_set=utf8 " + Conexion.dbname);
